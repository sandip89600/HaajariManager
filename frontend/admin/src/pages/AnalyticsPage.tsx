...
export default function AnalyticsPage() {
  const [activeTab, setActiveTab] = useState<
    "overview" | "business" | "workers" | "health"
  >("overview");
  ...
  useEffect(() => {
    ...
    const handleActivity = (event: any) => {
      ...
      if (
        event.action?.includes("REGISTER") ||
        event.action?.includes("SIGNUP")
      ) {
        triggerBlink("users");
        queryClient.invalidateQueries({ queryKey: ["adminAnalyticsFull"] });
      } else if (
        event.action?.includes("WORKER_ADD") ||
        event.action?.includes("WORKER_CREATE")
      ) {
        ...
        event.action?.includes("SUBSCRIPTION")
      ) {
  ...
  const uniqueOrgs = useMemo(() => {
    ...
    analytics.activityFeed.forEach((item: any) => {
      if (item.organization && item.organization !== "N/A")
  ...
  const exportData = (type: "csv" | "excel" | "pdf") => {
    ...
    if (type === "csv" || type === "excel") {
      ...
      link.setAttribute(
        "download",
        `Haajari_Metrics_${Date.now()}.${type === "csv" ? "csv" : "xls"}`,
      );
    ...
    return (
      <div className="flex flex-col justify-center items-center h-[70vh] space-y-4">
        ...
        <p className="text-slate-400 text-sm">
          Computing platform statistics from live MongoDB indices...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-extrabold text-white tracking-tight">
              Enterprise Analytics & Monitoring Center
            </h1>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Platform-wide system health, operational activity, telemetry audits,
            and SaaS MRR growth.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
        {/* Total Users */}
        <div
          className={`glass-card p-5 rounded-2xl border transition-all duration-500 ${realtimeBlinks.users ? "border-orange-500 bg-orange-500/10 scale-102 shadow-lg shadow-orange-500/15" : "border-slate-800/80 bg-slate-900/60"}`}
        >
          <div className="flex justify-between items-start">
            <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">
              Registered Users
            </span>
            <span className="p-2 rounded-xl bg-orange-500/10 text-orange-450 border border-orange-500/15">
          ...
          <div className="mt-4">
            ...
            <span className="text-xs text-orange-450 font-semibold flex items-center gap-0.5 mt-1.5">
              +{metrics.newUsersToday} created today{" "}
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800/80 bg-slate-900/60 transition-all duration-300">
          <div className="flex justify-between items-start">
            ...
            <span className="p-2 rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/15">
        ...
        {/* Workers Status */}
        <div
          className={`glass-card p-5 rounded-2xl border transition-all duration-500 ${realtimeBlinks.workers ? "border-emerald-500 bg-emerald-500/10 scale-102 shadow-lg shadow-emerald-500/15" : "border-slate-800/80 bg-slate-900/60"}`}
        >
          <div className="flex justify-between items-start">
            <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">
          ...
          <div className="mt-4">
            ...
            <span className="text-xs text-emerald-400 font-semibold flex items-center gap-0.5 mt-1.5">
              +{metrics.newWorkersThisMonth} registered this month{" "}
              <ArrowUpRight className="w-3 h-3" />
            </span>
          </div>
        </div>
        ...
        <div
          className={`glass-card p-5 rounded-2xl border transition-all duration-500 ${realtimeBlinks.attendance ? "border-violet-500 bg-violet-500/10 scale-102 shadow-lg shadow-violet-500/15" : "border-slate-800/80 bg-slate-900/60"}`}
        >
          <div className="flex justify-between items-start">
            <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">
              Today's Attendance
            </span>
            <span className="p-2 rounded-xl bg-violet-500/10 text-violet-400 border border-violet-500/15">
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800/80 bg-slate-900/60 transition-all duration-300">
          <div className="flex justify-between items-start">
            ...
            <span className="p-2 rounded-xl bg-rose-500/10 text-rose-450 border border-rose-500/15">
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-300">
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4 lg:col-span-2">
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Database className="w-5 h-5 text-orange-500" /> MongoDB
                Document Registry
              </h3>
              <p className="text-slate-500 text-xs mt-0.5">
                Real-time counts of Mongoose schemas in active cluster
                collections
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {Object.entries(dbStats || {}).map(([key, stat]: any) => (
                <div
                  key={key}
                  className="bg-slate-950/60 p-4 rounded-xl border border-slate-850/60 flex flex-col justify-between"
                >
                  ...
                  <div className="flex justify-between items-baseline mt-3">
                    ...
                    <span className="text-2xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/10 px-1.5 py-0.5 rounded font-mono">
                      OK
                    </span>
                  </div>
                </div>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <MapPin className="w-5 h-5 text-orange-500" /> Geographic
                Footprint
              </h3>
              <p className="text-slate-500 text-xs mt-0.5">
                Active workforce distribution by region
              </p>
            </div>
              ...
              <div className="border-t border-slate-800/80 pt-4">
                <div className="flex justify-between items-center text-xs text-slate-500">
                  ...
                  <span className="font-bold text-white font-mono">
                    {geoAnalytics.gpsAttendanceCount}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Smartphone className="w-5 h-5 text-orange-500" /> Device & OS
                Telemetry
              </h3>
              <p className="text-slate-500 text-xs mt-0.5">
                Active supervisor client configurations
              </p>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4 lg:col-span-2">
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                ...
                Live Exceptions & Audits
              </h3>
              <p className="text-slate-500 text-xs mt-0.5">
                Exceptions, authorization alerts, and validation warnings in
                database logs
              </p>
            </div>
                  ...
                  {errors && errors.length > 0 ? (
                    errors.map((err: any, idx: number) => (
                      <tr key={idx} className="hover:bg-slate-800/20">
                        <td className="py-2.5 font-mono text-rose-450">
                          {err.apiName}
                        </td>
                        <td className="py-2.5">{err.user}</td>
                        <td className="py-2.5 truncate max-w-xs">
                          {err.message}
                        </td>
                    ...
                    <tr>
                      ...
                      >
                        No active system exceptions recorded today.
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {activeTab === "business" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-300">
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white">
                New Supervisor Registrations
              </h3>
              <p className="text-slate-550 text-xs mt-0.5">
                Account completions over the last 7 calendar days
              </p>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              ...
              <p className="text-slate-555 text-xs mt-0.5">
                Calculated subscription rates accumulated over the last 6 months
              </p>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white">
                Client Subscription Breakdown
              </h3>
              <p className="text-slate-500 text-xs mt-0.5">
                Active organization plan metrics
              </p>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-400">
                    Free Tier:{" "}
                    <strong className="text-white">{plans.free}</strong>
                  </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-400">
                    Basic Tier:{" "}
                    <strong className="text-white">{plans.basic}</strong>
                  </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-400">
                    Professional Tier:{" "}
                    <strong className="text-white">{plans.professional}</strong>
                  </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-400">
                    Business Tier:{" "}
                    <strong className="text-white">{plans.business}</strong>
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white">
                Payroll & Expense Telemetry
              </h3>
              <p className="text-slate-500 text-xs mt-0.5">
                SaaS income vs worker payroll totals
              </p>
            </div>
            ...
            <div className="space-y-4 mt-2">
              <div className="flex justify-between items-center bg-slate-950/60 p-4 rounded-xl border border-slate-850/60">
                <div>
                  ...
                  <span className="text-lg font-extrabold text-orange-450 mt-1 block">
                    ₹{metrics.totalRevenue}
                  </span>
                </div>
              ...
              <div className="flex justify-between items-center bg-slate-950/60 p-4 rounded-xl border border-slate-850/60">
                <div>
                  <span className="text-slate-400 text-xs font-bold block">
                    Active Workers Wages Paid
                  </span>
                  <span className="text-lg font-extrabold text-emerald-450 mt-1 block">
              ...
              <div className="flex justify-between items-center bg-slate-950/60 p-4 rounded-xl border border-slate-850/60">
                <div>
                  ...
                  <span className="text-lg font-extrabold text-violet-450 mt-1 block">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-300">
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              ...
              <p className="text-slate-500 text-xs mt-0.5">
                Active worker profiles classified by trades
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
              <div className="h-60 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={Object.entries(workersByCategory || {}).map(
                      ...
                      dataKey="value"
                    >
                      {Object.entries(workersByCategory || {}).map(
                        (entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={COLORS[index % COLORS.length]}
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              ...
              <p className="text-slate-500 text-xs mt-0.5">
                Audit counter of active, completed, and delayed worksite
                telemetry
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-2">
              <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-850/60 text-center">
                <span className="text-slate-500 text-2xs uppercase font-bold block">
                  Total Workspaces
                </span>
              ...
              <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-850/60 text-center">
                ...
                <span className="text-3xl font-extrabold text-rose-450 mt-1 block">
                  {metrics.delayedSites}
                </span>
              </div>
            </div>
          </div>
      ...
      {activeTab === "health" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-300">
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Cpu className="w-5 h-5 text-orange-500 animate-spin-slow" />{" "}
                Host Hardware Telemetry
              </h3>
              <p className="text-slate-500 text-xs mt-0.5">
                CPU load, free memory limits, and backend latency metrics
              </p>
            </div>
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">
                    MongoDB Persistent Storage Allocation
                  </span>
                  <span className="font-bold text-white">
              ...
              <div className="border-t border-slate-800/80 pt-4 space-y-2">
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">
                    Socket.IO gateway clients
                  </span>
                  <span className="font-bold text-white">
                    {systemHealth.connectedClients} connections
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl border border-slate-850 bg-slate-900/30 space-y-4 lg:col-span-2">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                ...
                <p className="text-slate-500 text-xs mt-0.5">
                  Live Socket.IO activity logs from connected supervisor mobile
                  clients
                </p>
              </div>
            ...
            <div className="h-96 overflow-y-auto border border-slate-850 bg-slate-950/60 rounded-xl p-4 space-y-3 font-mono text-xs scrollbar-thin">
              {filteredLogs && filteredLogs.length > 0 ? (
                filteredLogs.map((log: any) => (
                  <div
                    key={log.id}
                    className="border-b border-slate-900 pb-2 flex flex-col sm:flex-row sm:items-start justify-between gap-1 hover:bg-slate-900/20 px-2 py-1 rounded"
                  >
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        <span className="bg-orange-500/10 text-orange-450 border border-orange-500/10 px-1 rounded text-2xs uppercase font-bold">
                          {log.action || "ACTIVITY"}
                        </span>
