...
export default function SupportPage() {
  ...
  const faqs = [
    {
      q: "How do I upgrade a client organization plan?",
      a: 'Navigate to the Users or Organizations tab, click the "Upgrade" action button on the corresponding row, and select the plan tier (Basic, Super, Premium) in the modal popup.',
    },
    {
      q: "What are the GPS radius validation limits?",
      a: "GPS boundaries are defined globally inside the System Settings panel. Client sites enforce this geofencing radius parameter (e.g. 150 meters) when supervisors mark workers present.",
    },
    {
      q: "Where are database backups stored?",
      a: "System backup configurations are integrated into the global setting profiles. Full database dumps can be automatically synced to Amazon S3 buckets or downloaded locally.",
    },
  ];
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Audit active client inquiries, system bugs, and administrative
            assistance logs
          </p>
        </div>
        <button
              ...
              filteredTickets.map((t) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-450 mt-1.5 leading-normal max-w-lg">
                      {t.issue}
                    </p>
                    <span className="block text-[10px] text-slate-550 mt-1 font-semibold">
                      Logged: {t.createdAt}
                    </span>
                  </div>
                  <span
        ...
        <div className="glass-card p-6 rounded-2xl border border-slate-850 h-fit space-y-4">
          ...
          <div className="space-y-3">
            {faqs.map((faq, idx) => (
              <div
                key={idx}
                className="border border-slate-850 rounded-xl overflow-hidden"
              >
                <button
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 text-slate-450 transition-transform ${openFaq === idx ? "rotate-180" : ""}`}
                  />
                </button>
                {openFaq === idx && (
