...
export default function WorkersPage() {
  ...
  const [filterCategory, setFilterCategory] = useState<WorkerCategory | "All">(
    "All",
  );
  ...
  const categories: (WorkerCategory | "All")[] = [
    "All",
    "Mason",
    "Helper",
    "Painter",
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Monitor global worker registries and wage contracts
          </p>
        </div>
        <button
          onClick={() => {
                ...
                filteredWorkers.map((w) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-extrabold border ${CATEGORY_COLORS[w.category]}`}
                      >
                        {w.category}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-bold ${!w.isArchived ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
                      >
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => {
                          if (
                            window.confirm(
                              `Are you sure you want to remove worker "${w.name}" permanently?`,
                            )
                          ) {
