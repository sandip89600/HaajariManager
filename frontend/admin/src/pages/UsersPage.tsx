...
import { Search, ChevronDown, CheckCircle, ShieldAlert, Clock, RefreshCw, Trash2, Eye, X, Building2, UserCheck, HardHat, Shield } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const handleOpenUserDetail = async (user: UserRecord) => {
    ...
    try {
      ...
      if (user.role === 'contractor') endpoint = `/admin/contractors/${user._id}`;
      if (user.role === 'supervisor') endpoint = `/admin/supervisors/${user._id}`;
                  ...
                  return (
                    <tr key={user._id} className="hover:bg-slate-900/20 transition-colors">
                      <td className="px-6 py-4 font-bold text-white">
                        <div>
                          <div>{user.name}</div>
                          <div className="text-xs text-slate-500 font-normal">{user.email || 'No Email'}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-semibold">
                        <span className={`px-2.5 py-1 rounded-lg text-xs font-extrabold uppercase inline-flex items-center gap-1.5 ${
                          user.role === 'contractor' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' :
                          user.role === 'supervisor' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
                          user.role === 'labor' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                          'bg-purple-500/10 text-purple-400 border border-purple-500/20'
                        }`}>
                          {user.role === 'contractor' && <Building2 className="w-3 h-3" />}
                          {user.role === 'supervisor' && <UserCheck className="w-3 h-3" />}
                          {user.role === 'labor' && <HardHat className="w-3 h-3" />}
                          {user.role === 'admin' && <Shield className="w-3 h-3" />}
                          {user.role}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-semibold text-slate-300">{user.phone}</div>
                        <div className="text-xs text-slate-500">@{user.username || 'n/a'}</div>
                      </td>
                      <td className="px-6 py-4 font-bold text-white">
                        {user.tenantId?.name || user.contractorCompany || user.contractorName || 'N/A'}
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                          user.connectionStatus === 'connected' || user.role === 'contractor' ? 'text-emerald-400 bg-emerald-500/10' :
                          user.connectionStatus === 'pending' ? 'text-amber-400 bg-amber-500/10' :
                          'text-slate-400 bg-slate-800'
                        }`}>
                          {user.role === 'contractor' ? 'Owner' : (user.connectionStatus || 'Not Connected')}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="font-bold text-white text-xs">{date}</div>
            ...
            {isLoadingDetails ? (
              ...
              <div className="space-y-4 text-sm text-slate-300">
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-850 space-y-2">
                  ...
                  <div className="text-xs text-slate-400">Username: @{selectedUser.username || 'n/a'}</div>
                  <div className="text-xs text-slate-400">Phone: {selectedUser.phone}</div>
                  <div className="text-xs text-slate-400">Email: {selectedUser.email || 'N/A'}</div>
                </div>

                {selectedUser.role === 'contractor' && userDetailedData.supervisors && (
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-850 space-y-2">
                    <div className="font-bold text-white">Supervisors ({userDetailedData.supervisors.total})</div>
                    <div className="text-xs text-emerald-400">Connected: {userDetailedData.supervisors.connected}</div>
                    <div className="text-xs text-amber-400">Pending: {userDetailedData.supervisors.pending}</div>
                  </div>
                )}

                {selectedUser.role === 'supervisor' && (
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-850 space-y-2">
                    <div className="font-bold text-white">Contractor Connection</div>
