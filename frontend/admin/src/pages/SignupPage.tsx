...
const SignupPage: React.FC = () => {
  ...
  const onSignupSubmit = async (data: SignupFormInputs) => {
    ...
    try {
      ...
      toast.success(
        "Administrator account registered and logged in successfully!",
      );
      navigate("/dashboard");
    } catch (err: any) {
      setIsLoading(false);
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSignupSubmit)} className="space-y-4">
          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-slate-400 block ml-1">
              Full Name
            </label>
            ...
            {errors.name && (
              <span className="text-[9px] text-rose-400 block ml-1">
                {errors.name.message}
              </span>
            )}
          </div>
