...
export default function SiteDetailsScreen() {
  ...
  const isContractorOrAdmin =
    userRole === "contractor" || userRole === "admin" || userRole === "builder";
  ...
  const [activeModal, setActiveModal] = useState<
    "work" | "material" | "photo" | "gps" | "issue" | null
  >(null);
  ...
  const [issuePriority, setIssuePriority] = useState<"Low" | "Medium" | "High">(
    "Medium",
  );
    ...
    try {
      ...
      if (newUpdate) {
        ...
        setSite((prev) => {
          ...
          if (type === "work") {
            if (newUpdate.workType)
              updatedSite.currentWork = newUpdate.workType;
            if (newUpdate.progressPercent !== undefined)
              updatedSite.currentProgress = newUpdate.progressPercent;
          }
          return updatedSite;
        });
  ...
  const getWorkCategoryProgress = (workKey: string) => {
    const catUpdates = updates.filter(
      (u) => u.type === "work" && u.workType === workKey,
    );
  ...
  const getWorkCategoryStatus = (workKey: string) => {
    ...
    if (catUpdates.length > 0) {
      ...
      return isToday
        ? "Updated Today"
        : `Updated ${latestDate.toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}`;
    }
    return "Not Updated";
  };
  ...
  const handlePickPhotoSection = async (
    ...
    if (!permissionResult.granted) {
      Alert.alert(
        "Permission Denied",
        "Camera/Gallery access is required to add photos.",
      );
      return;
    }
  ...
  const supervisorName =
    typeof site.supervisor === "object"
      ...
      : site.supervisor || "Ramesh";
  const supervisorPhone =
    typeof site.supervisor === "object"
      ? site.supervisor?.phone
      : (site as any).supervisorPhone;
  ...
  const workUpdatedCount = WORK_CATEGORIES.filter((cat) => {
    const catUpdates = updates.filter(
      (u) => u.type === "work" && u.workType === cat.key,
    );
    if (catUpdates.length > 0) {
      const latestDate = new Date(catUpdates[0].timestamp);
      const today = new Date();
  ...
  const morningUploaded = Boolean(
    morningPhotoUri ||
      latestPhotoUpdate?.morningPhoto ||
      (latestPhotoUpdate?.photoUris && latestPhotoUpdate.photoUris.length > 0),
  );
  const eveningUploaded = Boolean(
    eveningPhotoUri ||
      latestPhotoUpdate?.eveningPhoto ||
      (latestPhotoUpdate?.photoUris && latestPhotoUpdate.photoUris.length > 1),
  );

  const latestWorkUpdate = updates.find((u) => u.type === "work");
  const lastUpdatedTimeStr = latestWorkUpdate?.timestamp
    ? new Date(latestWorkUpdate.timestamp).toLocaleTimeString("en-IN", {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "6:12 PM";
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, paddingHorizontal: 10 }}>
          <ThemedText style={styles.headerControlTitle}>
            Site Control
          </ThemedText>
          <ThemedText numberOfLines={1} style={styles.headerSiteTitle}>
            {site.name}
          </ThemedText>
        </View>
      ...
      >
        ...
        >
          ...
          <View style={styles.infoRowBlock}>
            ...
            <ThemedText style={styles.infoValueText}>
              {site.address || (site as any).location || "Not specified"}
            </ThemedText>
          </View>
          ...
          <View style={styles.infoRowWithAction}>
            ...
            <Pressable
              onPress={() => handleMakeCall(supervisorPhone || "9876543210")}
              style={[styles.callActionBtn, { backgroundColor: "#2563EB" }]}
            >
        ...
        >
          ...
          >
            ...
            <Text
              style={[styles.todayDateBadge, { color: theme.textSecondary }]}
            >
              {todayDateFormatted}
            </Text>
          </View>
          ...
          >
            ...
            <ThemedText
              style={{ fontSize: 24, fontWeight: "900", color: theme.primary }}
            >
              {site.currentProgress || 65}%
            </ThemedText>
          </View>

          <View
            style={[
              styles.largeProgressBarBg,
          ...
          <View
            style={[
              styles.divider,
              { backgroundColor: theme.border, marginVertical: 14 },
            ]}
          />
          ...
          >
            <View>
              ...
              <ThemedText style={styles.summarySubVal}>
                {workUpdatedCount} / 5
              </ThemedText>
            </View>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 2 }}>
                <Text
                  style={{
                    fontSize: 13,
                    fontWeight: "700",
                    color: morningUploaded ? "#16A34A" : "#64748B",
                  }}
                >
                ...
                <Text
                  style={{
                    fontSize: 13,
                    fontWeight: "700",
                    color: eveningUploaded ? "#16A34A" : "#64748B",
                  }}
                >
        ...
        >
          ...
          >
            ...
            >
              <Text
                style={{ fontSize: 11, fontWeight: "800", color: "#2563EB" }}
              >
                MAIN FEATURE
              </Text>
            </View>
        ...
        <View style={styles.quickGridContainer}>
          ...
          >
            ...
            <Text style={[styles.quickCardText, { color: theme.text }]}>
              Daily Logs
            </Text>
          </Pressable>
        ...
        >
          ...
          >
            <Text
              style={[styles.outlineActionBtnText, { color: theme.primary }]}
            >
              Open Materials
            </Text>
          </Pressable>
        ...
        >
          ...
          >
            ...
            >
              <Feather
                name={morningUploaded ? "check-circle" : "clock"}
                size={16}
                color={morningUploaded ? "#16A34A" : "#64748B"}
              />
              <Text
            ...
            >
              <Feather
                name={eveningUploaded ? "check-circle" : "clock"}
                size={16}
                color={eveningUploaded ? "#16A34A" : "#64748B"}
              />
              <Text
        ...
        >
          ...
          <Pressable
            ...
            style={[
              ...
              { borderColor: "#DC2626", marginVertical: 10 },
            ]}
          >
            <Feather
              name="alert-triangle"
              ...
              style={{ marginRight: 6 }}
            />
            <Text style={[styles.outlineActionBtnText, { color: "#DC2626" }]}>
              Report Issue
            </Text>
          </Pressable>
          ...
          <ThemedText style={styles.emergencyHeaderTitle}>
            EMERGENCY CONTACTS
          </ThemedText>
          <View style={styles.emergencyCardRow}>
            ...
            <Pressable
              onPress={() => handleMakeCall("100")}
              style={[styles.callActionBtn, { backgroundColor: "#DC2626" }]}
            >
          ...
          <View style={[styles.emergencyCardRow, { marginTop: 10 }]}>
            ...
            <Pressable
              onPress={() => handleMakeCall("108")}
              style={[styles.callActionBtn, { backgroundColor: "#16A34A" }]}
            >
              <Feather name="phone" size={14} color="#FFFFFF" />
              <Text style={styles.callActionBtnText}>Call</Text>
            </Pressable>
        ...
        >
            ...
            <View style={{ flex: 1, paddingHorizontal: 10 }}>
              <ThemedText numberOfLines={1} style={styles.headerTitle}>
                {selectedWorkCategory !== null
                  ? `${selectedWorkCategory} Site Logs`
                  : "Daily Site Logs"}
              </ThemedText>
          ...
          {selectedWorkCategory === null && (
            ...
            >
              ...
              >
                <ThemedText style={styles.summaryBoxHeaderTitle}>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          <ThemedText style={styles.workCategoryTitle}>
                            {cat.label}
                          </ThemedText>
                          <ThemedText style={styles.workCategorySubText}>
                            Current Progress:{" "}
                            <Text
                              style={{ fontWeight: "800", color: cat.color }}
                            >
                              {pct}%
                            </Text>
                          </ThemedText>
          ...
          {selectedWorkCategory !== null && (
            ...
            >
                  ...
                  >
                    <Text style={{ fontSize: 28 }}>
                      {WORK_CATEGORIES.find(
                        (c) => c.key === selectedWorkCategory,
                      )?.icon || "🧱"}
                    </Text>
                    <View>
                      <ThemedText style={styles.dedicatedWorkTitle}>
                        {selectedWorkCategory}
                      </ThemedText>
                      <ThemedText style={styles.dedicatedWorkSub}>
                        Today's Progress
                      </ThemedText>
                    </View>
                  </View>
              ...
              <Pressable
                ...
                style={[
                  styles.addDailyUpdateBtn,
                  { backgroundColor: theme.primary },
                ]}
              >
                <Feather name="plus-circle" size={20} color="#FFFFFF" />
                <Text style={styles.addDailyUpdateBtnText}>
                  + Add Daily Update
                </Text>
              </Pressable>
              ...
              >
                {getWorkCategoryHistory(selectedWorkCategory).length === 0 ? (
                  ...
                  getWorkCategoryHistory(selectedWorkCategory).map(
                    (item, idx) => {
                      ...
                      return (
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            ...
                            <Text
                              style={[
                                styles.historyNotesText,
                                { color: isDark ? "#CBD5E1" : "#334155" },
                              ]}
                            >
                              "
                              {item.workNotes ||
                                "Daily progress update logged."}
                              "
                            </Text>
                            <Text style={styles.historyUpdaterText}>
                              Logged by {updaterName}
                            </Text>
                          </View>
                        </View>
                      );
            ...
            >
              ...
              <ScrollView contentContainerStyle={styles.modalScroll}>
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.readOnlyDateText, { color: theme.text }]}
                  >
                  ...
                  <Pressable
                    onPress={() => {
                      ...
                      if (isNaN(pct) || pct < 0 || pct > 100) {
                        Alert.alert(
                          "Validation Error",
                          "Please specify progress percentage between 0 and 100.",
                        );
                        return;
                      }
            ...
            <ScrollView contentContainerStyle={styles.modalScroll}>
              ...
              <View style={styles.unitChipGrid}>
                {UNIT_OPTIONS.map((unit) => {
                  const isSelected =
                    materialUnit.toLowerCase() === unit.toLowerCase();
                  return (
                    <Pressable
                      key={unit}
              ...
              <View style={styles.futureHardwareBox}>
                <Pressable disabled={true} style={styles.disabledHardwareBtn}>
                  ...
                  <Text style={styles.disabledHardwareBtnText}>
                    Send to Hardware Shop
                  </Text>
                </Pressable>
                <Text style={styles.futureMaterialNotice}>
                  This is future material.
                </Text>
              </View>
            </ScrollView>
          </View>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <ScrollView contentContainerStyle={styles.modalScroll}>
              ...
              >
                <ThemedText style={styles.photoSectionTitle}>
                  MORNING SITE PHOTO
                </ThemedText>
                ...
                {morningPhotoUri ? (
                  <View style={styles.photoPreviewBox}>
                    <Image
                      source={{ uri: morningPhotoUri }}
                      style={styles.photoPreviewImg}
                    />
                    <View style={styles.photoMetaRow}>
                      <Text style={styles.photoTimeText}>
                        Time: {morningPhotoTime}
                      </Text>
                      <Pressable
                        onPress={() => setMorningPhotoUri("")}
                  ...
                  <View style={styles.photoUploadBtnRow}>
                    <Pressable
                      onPress={() => handlePickPhotoSection("morning", true)}
                      style={styles.photoUploadBtn}
                    >
                      <Feather name="camera" size={18} color="#2563EB" />
                      <Text style={styles.photoUploadBtnText}>
                        Upload Morning Photo
                      </Text>
                    </Pressable>
                    <Pressable
                      onPress={() => handlePickPhotoSection("morning", false)}
                      style={styles.photoUploadBtn}
                    >
              ...
              >
                ...
                {eveningPhotoUri ? (
                  <View style={styles.photoPreviewBox}>
                    <Image
                      source={{ uri: eveningPhotoUri }}
                      style={styles.photoPreviewImg}
                    />
                    <View style={styles.photoMetaRow}>
                      <Text style={styles.photoTimeText}>
                        Time: {eveningPhotoTime}
                      </Text>
                      <Pressable
                        onPress={() => setEveningPhotoUri("")}
                        style={styles.removePhotoBtn}
                      >
                        <Feather name="trash-2" size={14} color="#EF4444" />
                      </Pressable>
                  ...
                  <View style={styles.photoUploadBtnRow}>
                    <Pressable
                      onPress={() => handlePickPhotoSection("evening", true)}
                      style={styles.photoUploadBtn}
                    >
                      <Feather name="camera" size={18} color="#2563EB" />
                      <Text style={styles.photoUploadBtnText}>
                        Upload Evening Photo
                      </Text>
                    </Pressable>
                    <Pressable
                      onPress={() => handlePickPhotoSection("evening", false)}
                      style={styles.photoUploadBtn}
                    >
                      <Feather name="image" size={18} color="#2563EB" />
                      <Text style={styles.photoUploadBtnText}>Gallery</Text>
              ...
              <Pressable
                onPress={() => {
                  if (!morningPhotoUri && !eveningPhotoUri) {
                    Alert.alert(
                      "Validation Error",
                      "Please upload at least one site photo (Morning or Evening).",
                    );
                    return;
                  }
                  handleCreateUpdate("photo", {
                    morningPhoto: morningPhotoUri
                      ? {
                          uri: morningPhotoUri,
                          time: morningPhotoTime,
                          notes: morningPhotoNotes,
                        }
                      : null,
                    eveningPhoto: eveningPhotoUri
                      ? {
                          uri: eveningPhotoUri,
                          time: eveningPhotoTime,
                          notes: eveningPhotoNotes,
                        }
                      : null,
                    photoUris: [morningPhotoUri, eveningPhotoUri].filter(
                      Boolean,
                    ),
                  });
                }}
          ...
          >
            ...
            <ScrollView contentContainerStyle={styles.modalScroll}>
              <ThemedText style={styles.modalDescText}>
                Confirm your physical presence at the site to record location
                authenticity.
              </ThemedText>
              ...
              <Pressable
                onPress={() => {
                  if (!gpsCoords) {
                    Alert.alert(
                      "Validation Error",
                      "Please capture GPS location coordinates first.",
                    );
                    return;
                  }
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <ScrollView contentContainerStyle={styles.modalScroll}>
              ...
              <ThemedText
                style={[styles.label, { color: "#EF4444", fontWeight: "800" }]}
              >
                Emergency Contacts
              </ThemedText>
              <View
                style={[
                  styles.emergencyModalBox,
                  {
                    borderColor: theme.border,
              ...
              <Pressable
                onPress={() => {
                  if (!issueDescription.trim()) {
                    Alert.alert(
                      "Validation Error",
                      "Please describe the site issue.",
                    );
                    return;
                  }
